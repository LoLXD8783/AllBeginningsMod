...
namespace AllBeginningsMod.Common.Bases
{
    public abstract class GreatswordItem : ModItem, ICustomUseStyle
    {
        ...
        float Progress(Player player) => (float)player.itemAnimation / player.itemAnimationMax;
        protected void SetArcRotation(Projectile projectile, Player player) {
            projectile.rotation = player.Center.DirectionTo(Main.MouseWorld).ToRotation() + (MathF.Sin(MathF.Pow(Progress(player), 2) * MathHelper.Pi) * MathHelper.Pi - MathHelper.PiOver2) * player.direction;
