...
namespace AllBeginningsMod.Common.PrimitiveDrawing
{
    internal class DefaultTrailStyle : ITrailStyle
    {
        ...
        public void SetBuffers(Vector2[] trailPositions, Func<float, float> trailWidth, Func<Vector2, Color> trailColor, ref VertexPositionColorTexture[] vertices, ref ushort[] indices) {
            Vector2 vertexOffset = trailPositions[0].DirectionTo(trailPositions[1]).RotatedBy(MathHelper.PiOver2) * trailWidth(0f) * 0.5f;
            vertices[0] = new VertexPositionColorTexture((trailPositions[0] - vertexOffset).ToVector3(), trailColor(Vector2.Zero), Vector2.Zero);
            vertices[1] = new VertexPositionColorTexture((trailPositions[0] + vertexOffset).ToVector3(), trailColor(Vector2.UnitY), Vector2.UnitY);
            ...
            for (int i = 1; i < trailPositions.Length; i++) {
                ...
                vertices[i * 2] = new VertexPositionColorTexture((trailPositions[i] - vertexOffset).ToVector3(), trailColor(new Vector2(factor, 0f)), new Vector2(factor, 0f));
                vertices[i * 2 + 1] = new VertexPositionColorTexture((trailPositions[i] + vertexOffset).ToVector3(), trailColor(new Vector2(factor, 1f)), new Vector2(factor, 1f));
